package jp.go.aist.airc.dkirt.rdfstar2vec;

import jp.go.aist.airc.dkirt.rdfstar2vec.debugging.VocabularyAnalyzer;
import jp.go.aist.airc.dkirt.rdfstar2vec.training.Gensim;
import jp.go.aist.airc.dkirt.rdfstar2vec.training.Word2VecConfiguration;
import jp.go.aist.airc.dkirt.rdfstar2vec.training.Word2VecType;
import jp.go.aist.airc.dkirt.rdfstar2vec.util.*;
...
public class Main {
     ...
     */
    public static final double DEFAULT_QT2SUBJECT = 0.5;
    
    /**
     * The transition probability from a quoted triple (QT) node to its constituent subjects.
     */
    ...
    public static void main(String[] args) {
        ...
        String qt2subjectText = getValue("-qt2subject", args);
        if (qt2subjectText != null) {
            try {
                probabilityFromQtToSubject = Double.parseDouble(qt2subjectText);
            } catch (NumberFormatException nfe) {
                System.out.println("Could not parse the probability. Using default (" + DEFAULT_QT2SUBJECT + ").");
                probabilityFromQtToSubject = DEFAULT_QT2SUBJECT;
            }
        } else probabilityFromQtToSubject = DEFAULT_QT2SUBJECT;
        System.out.println("Using trainsition probability from qt to subject " + Double.toString(probabilityFromQtToSubject));
        
        String object2qtText = getValue("-object2qt", args);
        if (object2qtText != null) {
            try {
                probabilityFromObjectToQt = Double.parseDouble(object2qtText);
            } catch (NumberFormatException nfe) {
                System.out.println("Could not parse the probability. Using default (" + DEFAULT_OBJECT2QT + ").");
        ...
        if (lightEntityFile != null) {
            walkGenerationMode = (walkGenerationMode == null) ? WalkGenerationMode.STAR_MID_WALKS_DUPLICATE_FREE : walkGenerationMode;
        } else {
            walkGenerationMode = (walkGenerationMode == null) ? WalkGenerationMode.STAR_MID_WALKS_DUPLICATE_FREE : walkGenerationMode;
        }
        ...
        if (isOnlyWalks) {
            ...
            if (lightEntityFile != null) {
                ...
                WalkGenerationManager classicGenerator = new WalkGenerationManager(
                		knowledgeGraphFile.toURI(),
                        isEmbedText,
                        true, 
                        existingWalkDirectory, 
